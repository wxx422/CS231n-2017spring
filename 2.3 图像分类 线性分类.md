...
线性分类器的另一个观点是回归到图像作为点和高维空间的概念。你可以想象每一张图片都是类似高维空间中的一个点。现在这个线性分类器在这些线性决策边界上尝试画一个线性分类面来划分一个类别和剩余其他类别。在左上角我们看到用来训练的飞机样例，通过训练过程，这个线性分类器会尝试绘制这条蓝色直线把飞机这个类别划分出来。如果你在训练过程中注意观察，这些线条会随机地开始然后快速变化，试图将数据正确区分开。
